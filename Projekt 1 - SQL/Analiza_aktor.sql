...
--- Czy aktor ma wp³yw na dochód z filmu i dochód z wypo¿yczeñ ?
...
select 
	...
	a.last_name as nazwisko,
	sum(p.amount) as przychód,
	count(DISTINCT(f.film_id)) AS liczba_filmow,
	round(sum(p.amount) / count(DISTINCT(f.film_id)),2) as œredni_przychod_z_filmu_dla_danego_aktora,
	count(DISTINCT(i.inventory_id )) AS liczba_kopii,
	count(DISTINCT(r.rental_id)) AS liczba_wypo¿yczen,
	round(count(DISTINCT(r.rental_id))/count(DISTINCT(f.film_id)), 2) as œrednia_liczba_wypo,
	round(sum(p.amount) / count(DISTINCT(r.rental_id)),2) as œredni_przychod_z_wypo¿yczenia
...
from actor a 
	...
	join payment p on r.rental_id = p.rental_id
		group by 1,2,3
		order by œredni_przychod_z_filmu_dla_danego_aktora desc;
...
------
	...
	count(DISTINCT(r.rental_id)) AS liczba_wypo¿yczen,
	round(count(DISTINCT(r.rental_id))/count(DISTINCT(f.film_id)), 2) as œrednia_liczba_wypo,
	round(sum(p.amount) / count(DISTINCT(r.rental_id)),2) as œredni_przychod_z_filmu
from actor a 
	join film_actor fa on a.actor_id = fa.actor_id 
	join film f on fa.film_id = f.film_id 
	join inventory i on f.film_id = i.film_id 
	...
	join payment p on r.rental_id = p.rental_id
		group by 1,2,3
		order by liczba_wypo¿yczen desc;
